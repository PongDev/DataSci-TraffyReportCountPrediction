...
@app.get("/predict")
def predict():
    region_list = ['กรุงธนเหนือ','กรุงเทพกลาง','กรุงธนใต้','กรุงเทพตะวันออก','กรุงเทพใต้','กรุงเทพเหนือ']
    rinfo = {
    ...
    for i,reg in enumerate(region_list):
        df_r = df[df['region']==reg].sort_values('date').drop(columns=['region','date'])
    ...
    for i,reg in enumerate(region_list):
        ...
        arr.append(i)
        arr.append(int(datetime.now().weekday()==5 or datetime.now().weekday()==6))
        input.append(arr)
    ...
    for day in range(1,3):
        ...
        prediction_road = road_model.predict(input)
        for i,reg in enumerate(region_list):
            response[reg+f'_sidewalk_{day}']=prediction_sidewalk[input[:,-2]==i][0]
            response[reg+f'_light_{day}']=prediction_light[input[:,-2]==i][0]
            response[reg+f'_road_{day}']=prediction_road[input[:,-2]==i][0]
    return response
